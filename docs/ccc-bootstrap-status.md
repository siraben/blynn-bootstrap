...
| 3 | `mlc-seed.m2` | `mlc/mlc-seed.c`; core fixtures under `tests/mlc`; flake targets `mlc-seed.host`, `mlc-seed.m2`, and `tests.mlc.seed.host-vs-m2`; host and M2 seed compilers emit byte-identical `.mzbc` for the seed-core fixture corpus and `mzvm-seed` runs those fixtures | partial: seed gate now targets the small core; implementation still contains transitional ADT/match parsing that should be removed once `mlc.ml` owns that parser/compiler |
