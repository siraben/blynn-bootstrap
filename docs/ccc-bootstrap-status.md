...
| 6 | `ccc.byte` | `ccc/ccc.ml`; `ccc/ccc.byte`; flake targets `ccc.byte.seed` and `tests.ccc.byte.committed`; fixed-point `mlc.byte` compiles `ccc.ml` to bytecode, then `mzvm-seed` runs that bytecode on Mes scaffold inputs `01-return-0.c` through `06-return-void.c`, plus an inline `return 42` fixture, and compares deterministic amd64 M1 output | partial: first real C-to-M1 compiler slice, limited to comments/whitespace, `int main(){return N;}`, constant-return calls, one/two-argument evaluated calls, local initialized integers, void no-op calls, `+`, unary `-`, and unary `!` over constants/calls |
