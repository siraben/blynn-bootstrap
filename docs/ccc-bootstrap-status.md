...
| 4 | committed `mlc.byte` | `mlc/mlc.byte`; flake targets `mlc.byte.seed` and `tests.mlc.byte.committed`; the committed bytecode matches M2-seed output for the current core-language `mlc/mlc.ml` lexer/parser/emitter spine; run under `mzvm-seed`, it accepts `write_byte (40+39)`, `write_byte (80 - 1)`, `write_byte (79 * 1)`, `write_byte (158 / 2)`, `write_byte 'O'`, `write_string "OK"`, recursively nested and parenthesized `let` byte-output fixtures with shadowing, and true/false `if ... then ... else ...` fixtures covering the current integer comparisons, emits `.mzbc`, and those emitted bytecode files print the expected bytes; the `let` and `if` paths emit VM variable, arithmetic, comparison, and branch instructions instead of just folding a byte constant | partial: tiny compiler-shaped bytecode only, not self-hosted |
...
2. Promote a compiler stage only when it can compile its own source and the
   next stage source. Stage 02 now compiles the current next source
   (`mlc/mlc.ml`), but not its own higher-order source.
3. Move ADT declarations, pattern parsing, and pattern compilation into
   `mlc.ml` instead of strengthening the C seed further.
4. Split tests into seed-core fixtures and, once `mlc.byte` can compile them,
   full-language ADT/pattern fixtures.
5. Only after the core seed can compile `mlc.ml`, commit `mlc.byte` and add the
