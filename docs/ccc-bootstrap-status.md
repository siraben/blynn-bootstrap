...
| 4 | committed `mlc.byte` | `mlc/mlc.byte`; flake targets `mlc.byte.seed` and `tests.mlc.byte.committed`; the committed bytecode matches the stage-02 self-compiled ML handoff output for the current `mlc/mlc.ml` lexer/parser/emitter spine; run under `mzvm-seed`, it accepts `write_byte (40+39)`, `write_byte (80 - 1)`, `write_byte (79 * 1)`, `write_byte (158 / 2)`, `write_byte 'O'`, `write_string "OK"`, recursively nested and parenthesized `let` byte-output fixtures with shadowing, true/false `if ... then ... else ...` fixtures covering the current integer comparisons, and the first two-arm ADT/pattern fixtures (`adt.ml`, `match.ml`, `wildcard-match.ml`), emits `.mzbc`, and those emitted bytecode files print the expected bytes; the `let`, `if`, constructor, and `match` paths emit VM variable, arithmetic, comparison, block, tag, and branch instructions instead of just folding byte constants | partial: staged ML-produced compiler-shaped bytecode with a first ADT/match slice, not self-hosted |
...
3. Continue growing ADT declarations, pattern parsing, and pattern compilation
   in `mlc.ml`; the current committed bytecode covers two-arm constructor and
   wildcard matches but not recursive/nested decision trees yet.
