...
| 3 | `mlc-interp-seed.m2` | `mlc/mlc-interp-seed.c`; stage sources under `mlc/stages`; flake targets `mlc-interp-seed.host`, `mlc-interp-seed.m2`, `mlc-stage-00-core`, `mlc-stage-01-parenthetical`, and `tests.mlc.interp-seed.host-vs-m2`; host and M2 interpreters produce identical output for the first named core stage; `01-parenthetical.ml` parses `02-ok.mzp`, emits `.mzbc`, and hands off to `mzvm-seed` | partial: tree-walking C root and first parenthetical handoff exist, but later full-language stages are not implemented |
