...
| 3 | `mlc-interp-seed.m2` | `mlc/mlc-interp-seed.c`; stage source `mlc/stages/00-core.ml`; flake targets `mlc-interp-seed.host`, `mlc-interp-seed.m2`, and `tests.mlc.interp-seed.host-vs-m2`; host and M2 interpreters produce identical output for the first named core stage | partial: tree-walking C root exists, but later ML stages are not implemented |
