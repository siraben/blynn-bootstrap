...
| 3 | `mlc-seed.m2` | `mlc/mlc-seed.c`; `tests/mlc/*.ml`; flake targets `mlc-seed.host`, `mlc-seed.m2`, and `tests.mlc.seed.host-vs-m2`; host and M2 seed compilers emit byte-identical `.mzbc` for the fixture corpus and `mzvm-seed` runs all fixtures | partial: small expression compiler, sequencing, `read_byte`/`write_byte`/`exit`, multi-character local bindings, direct unary `let rec` calls, two-element tuples, fixed-size arrays, declared unary/nullary ADTs, two-arm constructor matches with wildcard fallback, and literal string output |
| 4 | committed `mlc.byte` | `mlc/mlc.byte`; flake targets `mlc.byte.seed` and `tests.mlc.byte.committed`; the committed bytecode matches M2-seed output for the current `mlc/mlc.ml` placeholder, which now exercises ADTs, matching, tuples, local names, and literal output, and runs under `mzvm-seed` | partial: committed smoke bytecode only |
