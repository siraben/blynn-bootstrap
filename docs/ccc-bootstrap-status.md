...
| 4 | committed `mlc.byte` | `mlc/mlc.byte`; flake targets `mlc.byte.seed` and `tests.mlc.byte.committed`; the committed bytecode matches M2-seed output for the current core-language `mlc/mlc.ml` lexer/parser/emitter spine; run under `mzvm-seed`, it accepts `write_byte (40+39)`, `write_byte (80 - 1)`, `write_byte (79 * 1)`, `write_byte (158 / 2)`, `write_byte 'O'`, `write_string "OK"`, `let x = 40 in write_byte (x + 39)`, and true/false `if ... then ... else ...` byte-output fixtures including `<` and `==` conditions, emits `.mzbc`, and those emitted bytecode files print the expected bytes; the `let` and `if` paths emit VM variable, arithmetic, comparison, and branch instructions instead of just folding a byte constant | partial: tiny compiler-shaped bytecode only, not self-hosted |
