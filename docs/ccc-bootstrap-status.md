...
| 3 | `mlc-seed.m2` | `mlc/mlc-seed.c`; `tests/mlc/*.ml`; flake targets `mlc-seed.host`, `mlc-seed.m2`, and `tests.mlc.seed.host-vs-m2`; host and M2 seed compilers emit byte-identical `.mzbc` for the fixture corpus and `mzvm-seed` runs all fixtures | partial: small expression compiler, multi-character local bindings, declared unary/nullary ADTs, two-arm constructor matches, and literal string output |
