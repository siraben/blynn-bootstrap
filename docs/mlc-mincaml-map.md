...
ADT/pattern slice. Leading `type` declarations build a constructor
environment, constructor expressions allocate VM blocks, and simple
constructor/wildcard `match` forms lower to tag tests and branches, including
multi-declaration and three-arm direct matches. The next meaningful step is to
extend that ML-side parser/lowerer to recursive and nested decision-tree
