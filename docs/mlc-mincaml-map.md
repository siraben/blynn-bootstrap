...
destructuring, direct unary `let rec` calls, fixed-size `Array.create`, literal-index `a.(i)` reads and
`a.(i) <- v` writes, simple top-level ADT declarations of the form
