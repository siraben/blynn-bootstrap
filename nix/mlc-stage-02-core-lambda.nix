...
stageRun {
  ...
  buildScript = ''
    ...
    printf '(257 (write-byte (read-char)))' | ${mzvmSeedM2}/bin/mzvm-seed 03-core-lambda.mzbc > 03-core-read-char.mzbc
    actual="$(printf '\047O\047' | ${mzvmSeedM2}/bin/mzvm-seed 03-core-read-char.mzbc)"
    test "$actual" = O

    printf '(257 (write-byte (read-char)))' | ${mzvmSeedM2}/bin/mzvm-seed 03-core-lambda.mzbc > 03-core-read-char-escape.mzbc
    actual="$(printf '\047\\t\047' | ${mzvmSeedM2}/bin/mzvm-seed 03-core-read-char-escape.mzbc)"
    test "$actual" = "$(printf '\t')"
    ...
    printf '(120 (seq (need-string "OK") (write-byte 89)))' | ${mzvmSeedM2}/bin/mzvm-seed 03-core-lambda.mzbc > 03-core-need-string.mzbc
    actual="$(printf OK | ${mzvmSeedM2}/bin/mzvm-seed 03-core-need-string.mzbc)"
    test "$actual" = Y
    set +e
    printf NO | ${mzvmSeedM2}/bin/mzvm-seed 03-core-need-string.mzbc
    status="$?"
    set -e
    test "$status" = 1
  ...
  installScript = ''
    ...
    install -Dm644 03-core-read-char-escape.mzbc "$out/share/mlc/stages/03-core-read-char-escape.mzbc"
