...
#if defined(__M2__)
	#define CELL_SIZE sizeof(unsigned)
#else
	#define CELL_SIZE 1
#endif
...
void run(FUNCTION get, FUNCTION put)
{
	...
	while(TRUE)
	{
		...
		{
			fputs("?", stderr);
			fputs(int2str(x, 10, TRUE),stderr);
			fputs("\n",stderr);
...
int main(int argc, char **argv)
{
	...
	while(option_index <= argc)
	{
		...
		{
			foreign_version = strtoint(argv[option_index + 1]);
			option_index = option_index + 2;
		}
