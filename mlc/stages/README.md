...
- `03-ast-compiler.ml` is the first successor source compiled by the committed
  ...
  bytes, char literals, integer `+` / `-` / `*` / `/`, sequencing, bool
  literals, conditionals whose guards must type as `bool`, `let`, top-level
  `let`, pair construction, pair destructuring, full-input consumption, and
  static type errors.
