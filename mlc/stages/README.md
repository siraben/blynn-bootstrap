...
- `03-core-lambda.ml0` is the intentionally small next handoff compiled by
  ...
  `p_between`, `p_number`, `p_char_lit`, `p_literal`, `p_form_tag`, `p_need`,
  and `expect_string` as the first compositional parser surface. Primitive
  token, raw, digit, fixed-string, and form-tag parsing use shared parser
  helpers rather than spelling their bind structure independently.
