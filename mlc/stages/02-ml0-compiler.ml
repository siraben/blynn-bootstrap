...
let rec emit_expect_char value =
  fun dummy ->
  let _ = emit_push 0 in
  let _ = emit_const value 0 in
  let _ = emit0 9 0 in
  let _ = emit1 13 19 0 in
  let _ = emit_const 0 0 in
  let _ = emit_call_prim 0 0 in
  let _ = emit1 11 14 0 in
  let _ = emit_const 1 0 in
  emit_call_exit 0
in
...
let rec number ch =
  ...
  let ch = skip_space ch in
  if is_digit ch then
    number_loop (ch - '0') read_byte kon
  else
    exit 1
in
...
let rec parse_escape ch =
  fun kon ->
  if ch = 'n' then kon '\n' read_byte else
  if ch = 't' then kon '\t' read_byte else
  if is_digit ch then
    let d1 = ch - '0' in
    let c2 = read_byte in
    let c3 = read_byte in
    if is_digit c2 then
      if is_digit c3 then
        kon (d1 * 100 + (c2 - '0') * 10 + c3 - '0') read_byte
      else
        exit 1
    else
      exit 1
...
let rec compile_string_loop count =
  ...
  if ch = '"' then
    ...
    if count = 0 then
      ...
      compile_string_loop (count + 1) (len + 6) (emit3 emit emit_push (emit_const value)) ch kon) ch kon
in
...
let rec compile mode =
  ...
  let rec finish_ident word =
    ...
    let rec tail left_len =
      ...
      if ch = '+' then
        ...
        let next = read_byte in
        let ch = if next = '=' then read_byte else next in
  ...
  if mode = 0 then
    ...
    if ch = 'l' then
      ...
      if word = 229130382 then
        ...
        let rec expect_string_loop total_len =
          ...
          fun ch ->
          if ch = '"' then
            compile 2 read_byte env funcs base (fun arg_len -> fun arg_emit -> fun ch ->
            kon (arg_len + total_len) (emit2 arg_emit total_emit) ch)
          else
            parse_char ch (fun value -> fun ch ->
            expect_string_loop (total_len + 45) (emit2 total_emit (emit_expect_char value)) ch)
        in
        expect_string_loop 0 (fun dummy -> 0) ch
      else
        finish_ident word ch)
    else if 'a' <= ch then
      if ch <= 'z' then
    ...
    if ch = '(' then
      ...
      let ch = expect_string "Array.create" ch in
