(* uncurry-compiler: ML2 -> parenthesized MZBC assembly, optimizing.
   Fifth ML bootstrap stage; a fork of pattern-compiler whose delta is
   CODE GENERATION only (the accepted language is exactly ML2). It is
   therefore NOT a conservative extension: its output differs from stage
   04's by design, so it is verified by behavior (same fixture outputs,
   byte-identical compiler outputs downstream) and by its own
   second-generation fixpoint instead of byte-comparison against 04.

   The delta over pattern-compiler — uncurried known calls:
     - every top-level "let f p1 .. pk = e" with k >= 2 also compiles an
       n-ary body (all k parameters in one frame, entered with one
       APPLY k) stored in a hidden global slot "f#"; the public slot
       keeps a small curried wrapper, so f as a VALUE (passed around or
       partially applied) behaves exactly as before
     - a call "f a1 .. ak" whose head resolves to such a global and is
       syntactically saturated (decided by a token lookahead, like the
       paren-tail scan) pushes the arguments and emits one APPLY k --
       or APPTERM k in tail position, which makes self-recursion a loop.
       This removes the k-1 intermediate closure allocations and k-1
       call round-trips that the curried chain pays per call.

   Usage: mzvm pattern-compiler.mzbc uncurry-compiler.ml out.mzs *)
...
(* uncurrying: arity of the n-ary fast path (0 = none) and the hidden
   global slot holding the n-ary closure, per public global *)
let garity = cell (array_make 512 0)
let gnary = cell (array_make 512 0)
let gcount = cell 0
     ...
     let rec cp i =
       if i < n then
         (array_set nn i (array_get names i);
          ...
          array_set na i (array_get (get garity) i);
          array_set ns i (array_get (get gnary) i);
          cp (i + 1)) in
     cp 0;
     set gnames nn;
     set gdefined nd;
...
(* ---- saturation lookahead ----
   count up to `limit` whole argument atoms ahead of the cursor without
   consuming them, mirroring what c_app_args would treat as arguments
   (constructor atoms swallow their own arguments). Saves and restores
   the token state like paren_tail_closed. *)

let skip_count_brackets () =
  let rec go d =
    if get tk = tk_eof then ()
    else if tok_is_punct "(" || tok_is_punct "[" || tok_is_punct "{" then
      (next_token (); go (d + 1))
    else if tok_is_punct ")" || tok_is_punct "]" || tok_is_punct "}" then
      (next_token (); if d > 1 then go (d - 1))
    else (next_token (); go d) in
  next_token ();
  go 1

let rec skip_count_atom () =
  (if tok_is_punct "!" then (next_token (); skip_count_atom ())
   else if tok_is_punct "(" || tok_is_punct "[" || tok_is_punct "{" then
     skip_count_brackets ()
   else if get tk = tk_ident && is_ctor_name (get tstr) then
     (let c = find_ctor (get tstr) in
      next_token ();
      let arity = if c >= 0 then array_get (get ctor_arity) c else 0 in
      if arity = 1 then (if starts_atom () then skip_count_atom ())
      else if arity > 1 then (if tok_is_punct "(" then skip_count_brackets ()))
   else next_token ());
  let rec projs () =
    if tok_is_punct "." then
      (next_token ();
       (if get tk = tk_ident then next_token ());
       projs ()) in
  projs ()

let atoms_ahead limit =
  scan_save ();
  let rec count n =
    if n >= limit then n
    else if starts_atom () then (skip_count_atom (); count (n + 1))
    else n in
  let r = count 0 in
  scan_restore ();
  r
...
(* one frame holding all np parameters (the n-ary fast path): the args
   are pushed left to right, so the last one is ACC 0 on entry *)
let enter_level_params params np =
  let l = get lev in
  (if l + 1 >= maxlev then die "functions nested too deeply");
  array_set lev_saved_depth l (get depth);
  array_set lev_vbase (l + 1) (get vcount);
  ...
  set depth np;
  let rec binds i =
    if i < np then (bind_local (array_get params i) i; binds (i + 1)) in
  binds 0
...
(* the n-ary body of a top-level k-ary function: all parameters live in
   one frame. Top-level bodies can only reach globals, so the closure
   captures nothing. *)
and compile_fun_nary params np =
  let lf = new_label () in
  let ls = new_label () in
  ebranch "branch" ls;
  elabel lf;
  let saved_exited = get exited in
  enter_level_params params np;
  set exited 0;
  c_expr 1;
  set exited saved_exited;
  let fl = get lev in
  exit_level ();
  elabel ls;
  (if array_get ccounts fl > 0 then die "n-ary body captured a variable");
  eclosure lf 0
     ...
     if is_ctor_name name then
       ...
       (resolve name;
        if get res_kind = 3 then
          (c_builtin (get res_idx);
           (if starts_atom () then
             die "builtin or constructor result cannot be applied");
           c_app_args t)
        else if get res_kind = 2 && not (tok_is_punct ".") &&
                (let g = get res_idx - gbase in
                 let ar = array_get (get garity) g in
                 ar >= 2 && atoms_ahead ar >= ar) then
          c_direct_call (get res_idx - gbase) t
        else
          (emit_var_load ();
           c_proj_rest ();
           c_app_args t)))
...
(* saturated call to a known k-ary global: push the k arguments and
   enter the n-ary body with one APPLY/APPTERM instead of k unary
   round-trips through intermediate closures *)
and c_direct_call g t =
  let ar = array_get (get garity) g in
  let gn = array_get (get gnary) g in
  let rec args i =
    if i < ar then
      (c_atom ();
       c_proj_rest ();
       ...
       args (i + 1)) in
  args 0;
  e1 "getglobal" (gbase + gn);
  if starts_atom () then
    (* over-application: the saturated call's result is applied on *)
    (e1 "apply" ar;
     bump (0 - ar);
     c_app_args t)
  else if t = 1 && not (op_follows ()) then
    (e2 "appterm" ar (get depth - ar);
     set exited 1;
     bump (0 - ar))
  else
    (e1 "apply" ar;
...
(* hidden global name for the n-ary closure: "f" -> "f#" ('#' cannot
   appear in an identifier, so it can never be resolved by source code) *)
let nary_name name =
  let n = bytes_length name in
  let b = bytes_create (n + 1) in
  let rec cp i = if i < n then (bytes_set b i (bytes_get name i); cp (i + 1)) in
  cp 0;
  bytes_set b n 35;
  b

(* curried wrapper for a k-ary function: builds the chain
   "fun a1 -> .. fun ak -> <push a1..ak; tail-call global gn>" by hand,
   leaving the outermost closure in acc. In level i's body, a1..a_(i-1)
   are ENVACC 1..i-1 and a_i is the parameter at ACC 0. *)
let emit_wrapper k gn =
  let rec level i =
    let lf = new_label () in
    let ls = new_label () in
    ebranch "branch" ls;
    ...
    (if i = k then
      (let rec pushes j =
         if j < k then
           ((if j < k - 1 then e1 "envacc" (j + 1) else e1 "acc" j);
            e0 "push";
            pushes (j + 1)) in
       pushes 0;
       e1 "getglobal" gn;
      ...
      (level (i + 1);
       e1 "return" 1));
    elabel ls;
    (* captures a1..a_(i-1), loaded in the enclosing frame *)
    let rec caps j =
      if j < i - 1 then
        ((if j < i - 2 then e1 "envacc" (j + 1) else e1 "acc" j);
         e0 "push";
         caps (j + 1)) in
    caps 0;
    eclosure lf (i - 1) in
...
let top_binding () =
  ...
  if is_unit = 1 then
    ...
    (let name = need_ident "expected a binding name" in
     ...
     if np >= 2 then
       (* n-ary fast path in a hidden slot + curried wrapper in the
          public slot; saturated calls resolve the arity through g *)
       (let gn = add_global (nary_name name) in
        array_set (get gdefined) gn 1;
        array_set (get garity) g np;
        array_set (get gnary) g gn;
        compile_fun_nary params np;
        e1 "setglobal" (gbase + gn);
        emit_wrapper np (gbase + gn);
...
let () =
  (if arg_count () < 2 then
    (err_str "usage: pattern-compiler in.ml out.mzs"; write_byte 2 10; exit 1));
