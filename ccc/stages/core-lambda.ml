(* core-lambda: Lambda-0 -> binary MZBC image, written in Lambda-0.
   ...
   Compiles the Lambda-0 dialect -- unary functions, ints, bytes, closures
   and nothing else -- straight to a runnable .mzbc file (no assembler
   stage), and compiles itself to a fixpoint on the seed interpreter.
   ...
   strict OCaml subset and runs unchanged on the seed interpreter):
     ...
     - string literals appear only as the immediate argument of a call
       ...
       builtin aliases of `bytes_length`/`bytes_get`, exactly as
       ml0-compiler does) because neither the seed interpreter nor the
       host-OCaml prelude predefines it.
     ...
     - local bindings are `let x = e in e` (no parameters) and
       `let rec f x = e in e` (exactly one parameter, self-recursive).
     - identifiers are at most 64 bytes long; `_` is rejected as a
       binder (`let _ = e in` is not Lambda-0; sequence with `;`).
     - reserved words of fuller MLs (and, match, with, land, ...) are
       rejected so non-Lambda-0 sources fail loudly.
...
let r_strok = 18                    (* 1 when a string literal may follow *)
...
let rec read_all h =
  ...
  if b >= 0 then
    ((if rg r_srclen >= 1048576 then (err_str "core-lambda: source too large"; die 0));
...
let rec skip_comment depth =
  if depth > 0 then
    (let c = nextc 0 in
     (if c < 0 then (err_str "core-lambda: unterminated comment"; die 0));
...
let read_escape = fun u ->
  ...
  else (err_str "core-lambda: bad escape"; die 0)

let tpush = fun b ->
  let n = rg r_tlen in
  (if n >= 250 then (err_str "core-lambda: token too long"; die 0));
  bytes_set tbuf n b;
  rs r_tlen (n + 1)
...
let next_token = fun u ->
  ...
  if c < 0 then rs r_tk 0
    ...
    (let rec dec acc =
     ...
     let v = dec 0 in
     (if is_ichar (peekc 0) then (err_str "core-lambda: bad number"; die 0));
     rs r_tint v;
     rs r_tk 1)
    ...
    (rs r_tlen 0;
     let rec idloop u2 =
       if is_ichar (peekc 0) then
         ((if rg r_tlen >= 64 then (err_str "core-lambda: identifier too long"; die 0));
          tpush (nextc 0);
          idloop 0) in
     idloop 0;
    ...
    (ign (nextc 0);
     ...
     let rec sloop u2 =
       let d = peekc 0 in
       if d < 0 then (err_str "core-lambda: unterminated string"; die 0)
...
let is_errstr_tok = fun u ->
  rg r_tk = 3 && rg r_tlen = 7
  && rg r_ik0 = 356493149 && rg r_ik1 = 18 (* err_str *)
...
let need_ident = fun u ->
  if at_ident 0 then
    ((if rg r_tlen = 1 && bytes_get tbuf 0 = 95 then
        (err_str "core-lambda: _ is not a Lambda-0 binder; sequence with ; instead"; die 0));
     take_ident 0)
  else (err_str "core-lambda: expected an identifier"; die 0)
...
let expect_p = fun c ->
  ...
  else (err_str "core-lambda: expected "; wp2 c; die 0)
...
let emitw = fun v ->
  let n = rg r_clen in
  (if n >= 1048575 then (err_str "core-lambda: code too large"; die 0));
...
let emit_strlit = fun u ->
  ...
  (if i >= gbase then (err_str "core-lambda: too many string literals"; die 0));
  let n = rg r_tlen in
  let o = rg r_datlen in
  (if o + 4 + n > 262144 then (err_str "core-lambda: data section too large"; die 0));
  bytes_set dat o n;
...
let add_global = fun q ->
  ...
  (if i >= 2048 then (err_str "core-lambda: too many globals"; die 0));
  let n = bytes_length q in
  let o = rg r_gplen in
  (if o + 2 + n > 65536 then (err_str "core-lambda: global name pool full"; die 0));
...
let cap_add = fun l -> fun q ->
  let n = rg (r_ccount0 + l) in
  (if n >= 64 then (err_str "core-lambda: too many captured variables"; die 0));
...
let load_res = fun r ->
  ...
  else (err_str "core-lambda: builtin used as a value"; die 0)
...
let c_atom = fun re -> fun env ->
  ...
  if k = 1 then (emit1 1 (rg r_tint); next_token 0)
    ...
    (if rg r_strok = 1 then (emit_strlit 0; rs r_strok 0; next_token 0)
     else (err_str "core-lambda: string literal outside err_str"; die 0))
  else if k = 3 then
    (if kw_at 126748 (* true *) then (emit1 1 1; next_token 0)
     else if kw_at 3499810 (* false *) then (emit1 1 0; next_token 0)
     else if is_kw_tok 0 then (err_str "core-lambda: unexpected keyword"; die 0)
  ...
  else (err_str "core-lambda: unexpected token"; die 0)
...
let c_builtin = fun re -> fun env -> fun d ->
  ...
  let rec args j =
    if j < ar then
      ((if bnot (starts_atom 0) then
          (err_str "core-lambda: builtin applied to too few arguments"; die 0));
       c_atom re env;
  ...
  if kind = 0 then (emit2 47 ar idx; bump (0 - ar))
    ...
    ((if idx = 1 then emitw 43 else emitw 44);   (* getbytes / setbytes *)
     bump (0 - (ar - 1)))
  ...
  if t = 1 && t_punct 40 && ptc 0 = 1 then
    ...
    (let was_builtin =
       if at_ident 0 && bnot (kw_at 126748) && bnot (kw_at 3499810) then
         (let es = if is_errstr_tok 0 then 1 else 0 in
          let name = take_ident 0 in
          let r = env name in
          if r / 1048576 = 3 then (c_builtin re env (r mod 1048576); 1)
          else
            (load_res r;
             rs r_strok (if es = 1 && rg r_tk = 2 then 1 else 0);
             0))
       else (c_atom re env; 0) in
     (if was_builtin = 1 && starts_atom 0 then
        (err_str "core-lambda: builtin result cannot be applied"; die 0));
     let rec app_args u =
...
(* compile `fun pname -> body` into a closure value; the body is emitted
   first (behind a branch) so its free variables are discovered, then the
   captured values are pushed and CLOSURE built *)
let compile_fun = fun re -> fun env -> fun pname ->
  let hs = hole 14 in                              (* branch over the body *)
  let lf = rg r_clen in
  let saved_ex = rg r_exited in
  let l = rg r_lev in
  (if l + 1 >= 64 then (err_str "core-lambda: functions nested too deeply"; die 0));
  rs (r_sdepth0 + l) (rg r_depth);
  rs (r_ccount0 + l + 1) 0;
  rs r_lev (l + 1);
  rs r_depth 1;
  rs r_exited 0;
  ign (re 0 (env_bind pname (mk_res 0 0) (boundary (l + 1) env)) 1);
...
let c_funexpr = fun re -> fun env ->
  next_token 0;
  let pname = need_ident 0 in
  (if at_ident 0 then
     (err_str "core-lambda: functions are unary; nest fun"; die 0));
  (if bnot (t_punct 15917) then (err_str "core-lambda: expected ->"; die 0));
  next_token 0;
  compile_fun re env pname

let c_if = fun re -> fun env -> fun t ->
  next_token 0;
  ign (re 0 env 0);
  (if bnot (kw_at 311492) then (err_str "core-lambda: expected then"; die 0));
  next_token 0;
  if t = 1 then
    ...
    (let d0 = rg r_depth in
     ...
     if then_ex = 1 && else_ex = 1 then rs r_exited 1
       ...
       ((if (then_ex = 1 || else_ex = 1) && t_punct 59 then
           (err_str "core-lambda: one branch of this if exits in tail position but the other falls into a sequence; parenthesize"; die 0));
        rs r_exited 0))
  else
    (let ha = hole 16 in
     ign (re 1 env 0);
...
let c_let = fun re -> fun env -> fun t ->
  ...
  if kw_at 2510 (* rec *) then
     ...
     let rec patch j =
       if j < nc then
         ((if cap_eq fl j name then
          ...
          patch (j + 1)) in
     patch 0;
     (if bnot (kw_at 401) then (err_str "core-lambda: expected in"; die 0));
     next_token 0;
     ign (re 0 env2 t);
    ...
    (let name = need_ident 0 in
     (if at_ident 0 then
        (err_str "core-lambda: local let takes no parameters; use fun"; die 0));
     expect_p 61;
     ...
     let rec seq u =
       if t_punct 59 then
         ((if rg r_exited = 1 then
             (err_str "core-lambda: tail branch followed by a sequence; parenthesize the if/let"; die 0));
          next_token 0;
          cexp 1 env t;
          seq 0) in
     seq 0)
...
let rec top_loop u =
  if rg r_tk = 0 then ()
    ...
    ((if bnot (kw_at 15832) then
        (err_str "core-lambda: expected a top-level let"; die 0));
     next_token 0;
     (if kw_at 2510 (* rec *) then
        (next_token 0;
         let name = need_ident 0 in
        ...
        (let name = need_ident 0 in
         (if at_ident 0 then
            (err_str "core-lambda: top-level let takes no parameters; use fun or let rec"; die 0));
         expect_p 61;
         let g = mark_defined name in
         cexp 0 genv 0;
         emit1 46 (gbase + g)));
...
let () =
  ...
  (if h < 0 then (err_str "core-lambda: cannot open input"; die 0));
  read_all h;
  close_chan h;
  next_token 0;
  top_loop 0;
  ...
  (if o < 0 then (err_str "core-lambda: cannot open output"; die 0));
  rs r_outh o;
