...
- **Algebraic data types and pattern matching.** CCC should compile proper
  ML-style variants and `match ... with` forms rather than forcing source
  programs to encode sums as `(tag, payload)` tuples by convention. Pattern
  matching is compiled explicitly in `mlc` to constructor-tag tests, field
  extraction, and decision trees in the generated bytecode. This adds some
  compiler work, but it makes the later `ccc.ml` port clearer because ASTs,
  tokens, C types, and diagnostics can use direct constructors.
...
| `Hcc.TypesAst`            | direct ADTs in mini-OCaml; pattern matching compiled by `mlc` |
