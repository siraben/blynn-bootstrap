...
- No GC (heap grows; long-running runs handle this via OS exit).
...
- **Variants — but encoded.** No ADTs in the language. Sum types are encoded
  as `(tag, payload)` tuples with integer tags and well-known constructors
  in a per-module convention. This is identical to MinCaml's approach and
  HCC already largely does this in Haskell.
...
**Explicitly out**
- Polymorphism, records, modules, functors, objects, exceptions, pattern
  matching beyond `let (x,y) = ...`, GC, floats (we do not need floats for
  the C-compiler path; we drop MinCaml's float pipe entirely), SPARC backend.
...
**Memory layout**
- One contiguous heap, bump-allocated, no GC (matches MinCaml).
...
| `Hcc.TypesAst`            | variants encoded as `(tag, payload)` tuples  |
