...
- `upstreams/gnu-mes-libc-hcc-bootstrap.patch`
  - upstream: `https://git.savannah.gnu.org/git/mes.git` at
    `c331d801da386ba752f3fe92d0538102a90e988d`
  - source compatibility delta used by the HCC-built TinyCC Mes libc path

The GNU Mes libc aggregate source order comes from Mes'
`build-aux/configure-lib.sh`. The aggregate `libc.c` itself is generated by
build stages rather than carried as a large generated patch.
