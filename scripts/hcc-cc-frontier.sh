...
append_pp_arg() {
  case "$1" in
    *'
'*) die "preprocessor argument contains a newline" ;;
  esac
...
'
  set -f
  if test -n "$pp_args"; then
    # Splitting only on newlines preserves spaces and shell metacharacters in
    # quoted string definitions with embedded spaces and punctuation.
...
cleanup() {
  status=$?
  if test "$status" -ne 0 && test "${HCC_KEEP_FAILED_TEMPS:-0}" = 1; then
    echo "hcc-cc-frontier: keeping failed intermediates in $work" >&2
  else
    rm -rf "$work"
  fi
}
