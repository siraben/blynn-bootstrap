...
Measured `hcc1 --m1-ir tcc-expanded.c` using `hcc1-blynn.c` generated by
stage0-built Precisely and compiled with GCC:

```text
TOP words   elapsed   max RSS
268435456   13.79s   ~2.1 GiB
134217728   14.35s   1061876 KiB
100663296   15.07s    796424 KiB
 83886080   15.88s    663304 KiB
 67108864   16.76s    531464 KiB
 50331648   18.56s    398856 KiB
 33554432   25.40s    266248 KiB
 25165824   38.11s    201224 KiB
 20971520   71.57s    165968 KiB
```

All successful runs produced the same HCC IR hash:

```text
83f36cd81157fa53fad93eabaaaab9299d6e6fa6edfa049cff724fc3668e8699
```

Change:

- Reduced generated-HCC GCC/TCC backend `hcc1Top` from `268435456` to
  `134217728`.

Conclusion:

`134217728` is the best runtime-first point in this sweep: it roughly halves peak
RSS while adding only about 4% runtime on expanded TinyCC. Lower heap sizes keep
working but quickly trade too much runtime for memory.

Validation:

```text
nix build .#hcc.m2.precisely.gcc .#tinycc.m1.m2.precisely.gcc --no-link --print-out-paths -L
pass, tcc-expanded.c and tcc.M1 hashes unchanged

nix build .#tinycc.m2.precisely.m2 --no-link --print-out-paths -L
pass, cached boss output unchanged

nix build .#hcc.gcc.precisely.tcc --no-link --print-out-paths -L
pass
```
